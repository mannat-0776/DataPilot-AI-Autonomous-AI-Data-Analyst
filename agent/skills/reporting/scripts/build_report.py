        ...
        for value_col in ranked_metrics[:2]:
            ...
            insights.append({
                ...
                "detail": (
                    ...
                    f"in {table_title} at {value}."
                ),
                "metric": readable_metric.title(),
                "value": value,
                "why": f"Primary metric driver identified from segment breakdown in {table_title}.",
                "evidence": f"Pivoted table records show {label} leading at {value} for {readable_metric}.",
                "confidence": 94,
                "recommended_action": f"Investigate underlying conversion and retention dynamics for {label}.",
                "investigate_prompt": f"Investigate {label}'s {readable_metric} performance and compare against secondary segments."
            })
    return insights
